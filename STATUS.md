...
- Fresh final branch Preview after the chat-only submit flow and mobile visual-test stabilization: `https://project-12-demo-staging-public-emg7rfbtf-tomupros-projects.vercel.app` (`dpl_Gge24mTKAXgzXtQpjeHWNKxKb5CE`). Anonymous `/` and `/api/health` both return 200; health reports `mode=demo`, `realMoneyDisabled=true`, `bot=blocked`, `database=disabled`.
...
- Latest final Preview deployment: `dpl_Gge24mTKAXgzXtQpjeHWNKxKb5CE`, URL `https://project-12-demo-staging-public-emg7rfbtf-tomupros-projects.vercel.app`, Vercel state `READY`; health confirms `mode=demo`, `bot=blocked`, `database=disabled`, and `realMoneyDisabled=true`.
