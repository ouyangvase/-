...
- The Mini App now has an authenticated internal chat feed with numeric and natural-language banker/bet commands, `shN`/`梭哈`, close-betting, SSE delivery and participant-only packet access; the UI is still a compact reference-derived implementation rather than a pixel copy of the supplied Telegram screenshots.
...
- The internal chat game path is implemented in Demo mode: verified users control the round only by sending text in the room. Numeric and natural commands such as `抢庄 600`, `下注 5`, `sh10`/`梭哈 50`, `封盘`, `确认发包`, `/重推`, `抢红包`/`claim`, and the `1`/`0` continuation commands are parsed server-side; there are no game-action buttons. `封盘` moves the round to `WAITING_BANKER_CONFIRM`, and the current banker can send any non-empty chat message to confirm the internal packet (or `/重推` to cancel). Only recorded bettors receive the private packet card and claim eligibility; spectators receive no claim entry point.
...
- The public Vercel deployment exposes the mobile-first Mini App and bundled API at `https://project-12-demo-staging-public.vercel.app`; the latest production deployment is `dpl_CUfVwTKLNx4BEcXWtRypaV7VSZ9C`, and direct `/api/health` plus `/` checks return 200. The current public runtime is explicitly Demo mode with real-money paths disabled.
- Fresh final branch Preview after text-only internal chat enforcement: `https://project-12-demo-staging-public-8ym6avfdp-tomupros-projects.vercel.app` (`dpl_CUfVwTKLNx4BEcXWtRypaV7VSZ9C`). Anonymous `/` and `/api/health` both return 200; health reports `mode=demo`, `realMoneyDisabled=true`, `bot=blocked`, `database=disabled`.
...
- This turn made internal chat messages database-first before realtime broadcast, while keeping every game action text-only through the single composer. The canonical `POST /api/chat/rooms/room-12/messages` route, full typed `下注 5` path, persisted round bettors, and participant-only packet visibility were verified; no game-action buttons were added.
...
- Latest production deployment: `dpl_CUfVwTKLNx4BEcXWtRypaV7VSZ9C`, alias `https://project-12-demo-staging-public.vercel.app`, Vercel state `READY`; health confirms `mode=demo`, `bot=blocked`, `database=disabled`, and `realMoneyDisabled=true`. The production bundle contains the canonical internal-chat message route and no legacy chat-command URL.
- Latest final Preview deployment: `dpl_CUfVwTKLNx4BEcXWtRypaV7VSZ9C`, URL `https://project-12-demo-staging-public-8ym6avfdp-tomupros-projects.vercel.app`, Vercel state `READY`; health confirms `mode=demo`, `bot=blocked`, `database=disabled`, and `realMoneyDisabled=true`.
...
| Runtime smoke | Demo API auth → device → referral → runtime scrypt PIN → bet → claim → settlement passed; public Vercel `/api/health` and `/api/health/live` return 200 after latest production deploy; live canonical chat smoke `抢庄 600` returned `BID` and `BANKER_BIDDING` |
