...
- Non-Demo API and Bot deployments fail closed when the admin token or Telegram webhook secret is absent; local Demo defaults are not accepted in staging.
...
| Staging auth gate | passed: `APP_MODE=staging`, mock disabled; unsigned initData returns 503 `TELEGRAM_SIGNED_INIT_DATA_REQUIRED`, default admin token returns 403 and missing webhook secret returns 503 `WEBHOOK_SECRET_REQUIRED` |
