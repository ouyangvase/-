...
- 最新本地提交：`ff7813b`
- 稳定公开地址：`https://project-12-demo-staging-public.vercel.app`
...
- 固定公开地址的 `/api/health` 可访问，当前明确报告为 `mode: demo`、`database: disabled`、`bot: blocked`，没有把 Demo 误报为生产。
- 固定公开地址已完成 Preview → endpoint 验证 → Production promote；`/api/version` 返回版本 `0.1.0-demo` 和提交 `ff7813b...`，API 使用 `no-store`，静态资源使用 hash + immutable 缓存策略。
...
- 本次 Vercel 生产别名与运行时证据已记录在 `docs/evidence/vercel-production-domain.json`；其中 Telegram screenshots 为空，避免把部署证据伪装成 Telegram 实机证据。
