...
- Integration: `scripts/api-smoke.mjs` covers HttpOnly cookie issuance, API idempotency, demo claim, settlement, disabled money routes and onboarding; the realtime probe verifies an initial SSE snapshot and a later state transition on the same stream. Run `API_URL=https://<api-host> pnpm test:staging-gate` to verify unsigned auth, default admin-token rejection and missing webhook-secret rejection against an explicit non-Demo staging API. The Bot staging check also verifies that `/health` returns 503 without a Bot token and that webhook requests fail closed without the configured secret.
