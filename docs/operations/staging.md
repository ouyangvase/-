...
- Mini App production alias: `https://project-12-demo-staging-public.vercel.app` (latest verified deployment `dpl_5pEXpdtWsWeob6B1PdBhVeAiiPNb`). Root, `/games/12/rules`, `/profile/referral` and an unknown SPA route return HTTP 200; the served bundle includes banker bidding, Telegram MainButton wiring, two-decimal settlement handling, the Telegram-runtime API-write guard, Telegram theme/viewport event updates, live SSE round updates and the SecureStorage compatibility warning.
