...
describe("internal chat game commands", () => {
  ...
  it("moves banker and bettor through confirmation and private packet stages", async () => {
    ...
    expect(canonicalRound.status).toBe(200);
    expect((await canonicalRound.json()).state).toBe("CLAIMING");
    const roomRealtimeSnapshot = await request("/api/chat/rooms/room-12/realtime?snapshot=1", { headers: bankerSession });
