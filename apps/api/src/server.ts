...
export const apiHandler = async (request: IncomingMessage, response: ServerResponse) => {
  try {
    ...
    if (request.method === "GET" && url.pathname === `/api/rounds/${state.round.id}/fairness`) return json(response, 200, { ruleVersion: demoRules.id, seedHash: hashSeed(serverSeed), revealed: ["ROUND_COMPLETE", "REFUNDED"].includes(state.round.state), serverSeed: ["ROUND_COMPLETE", "REFUNDED"].includes(state.round.state) ? serverSeed : undefined, inputTemplate: `${state.round.id}:<userId>:<claimSequence>` });
      if (request.method === "GET" && url.pathname === `/api/rounds/${state.round.id}/settlement`) return json(response, 200, { roundId: state.round.id, state: state.round.state, ledger: state.ledger.slice(0, 5), events: roundEvents.slice(0, 10), results: roundResults.get(state.round.id) ?? [], demoOnly: appMode === "demo" });
    ...
    if (request.method === "GET" && url.pathname === "/api/admin/rules") return requireAdmin(request) ? json(response, 200, [{ id: demoRules.id, version: demoRules.id, status: "ACTIVE", rules: demoRules, immutable: true, demoOnly: appMode === "demo" }]) : json(response, 403, { error: "Admin authorization required" });
