...
async function transition(to: RoundState, actor: string, payload: Record<string, unknown> = {}) {
  ...
  if (to === "BETTING" && typeof payload.amount === "number") addRoomMessage("BANKER", `${messageActor(String(payload.banker ?? actor))} 抢庄 ${Math.max(payload.amount, Number(payload.currentHighest ?? 0))} PT，当前进入下注阶段。`, { banker: payload.banker ?? actor, amount: payload.amount });
  if (to === "WAITING_BANKER_CONFIRM" && payload.bettingClosed === true) addRoomMessage("ROUND", `✅ 下注意结束，已记录本局 ${Number(payload.bettorCount ?? 0)} 位下注玩家。请庄家发送「确认发红包」开始发红包；旁观者不会收到领取入口。`, { bettorCount: payload.bettorCount, banker: payload.banker, packetMode: "INTERNAL" });
  if (to === "PACKET_SENT" && payload.bettingClosed === true) addRoomMessage("ROUND", `🎁 庄家已确认，平台红包已向本局 ${Number(payload.bettorCount ?? 0)} 位已下注玩家私发。旁观者不会收到领取入口。`, { amount: payload.amount, packetId: payload.packetId, bettorCount: payload.bettorCount, packetMode: "INTERNAL" });
  if (to === "EVALUATING" && typeof payload.claimedAt === "string") addRoomMessage("PACKET", `${messageActor(actor)} 已领取平台红包，进入算牌。`, { claimSequence: payload.claimSequence }, actor);
  if (to === "ROUND_COMPLETE") addRoomMessage("SETTLEMENT", `平台通知：回合已完成，结算结果已写入 Demo 账本。`, { outcome: payload.outcome });
...
export const apiHandler = async (request: IncomingMessage, response: ServerResponse) => {
  try {
    ...
    if (request.method === "POST" && ["/api/wallet/top-up", "/api/wallet/withdraw", "/api/wallet/deposit-requests", "/api/wallet/withdraw-requests", "/api/payments/charge", "/api/rewards/cash-out"].includes(url.pathname)) return rejectMoney(response);
