...
const roomRealtimeClients = new Set<ServerResponse>();
type AppLocale = "zh-CN" | "en" | "ms" | "th" | "vi" | "id";
const supportedLocales = new Set<AppLocale>(["zh-CN", "en", "ms", "th", "vi", "id"]);
...
const roomMessages: RoomMessage[] = [
  ...
  { id: "MSG-0003", type: "BANKER", body: "Mira 已抢庄 500 PT，当前进入下注阶段。", createdAt: now(), payload: { banker: "Mira", amount: 500 } }
];
...
const roundBettors = new Map<string, Set<string>>();
...
function broadcastRoomEvent(event: RoomMessage) { for (const response of roomRealtimeClients) if (!writeRoomEvent(response, event)) roomRealtimeClients.delete(response); }
...
async function transition(to: RoundState, actor: string, payload: Record<string, unknown> = {}) { const from = state.round.state; assertTransition(from, to); const stateEndsAt = stateDeadline(to); await persistence.persistRoundEvent({ roundId: state.round.id, from, to, stateEndsAt, payload: { ...payload, stateEndsAt: stateEndsAt?.toISOString() ?? null }, actor }); state.round.state = to; const event = { id: `RE-${String(roundEvents.length + 1).padStart(4, "0")}`, roundId: state.round.id, from, to, payload, createdAt: now() }; roundEvents.unshift(event); queueOutbox("ROUND_STATE_CHANGED", { ...event }); audit(actor, "ROUND_STATE_CHANGED", "ROUND", state.round.id, { state: from }, { state: to, ...payload }); if (to === "BETTING" && typeof payload.amount === "number") addRoomMessage("BANKER", `${messageActor(String(payload.banker ?? actor))} 抢庄 ${Math.max(payload.amount, Number(payload.currentHighest ?? 0))} PT，当前进入下注阶段。`, { banker: payload.banker ?? actor, amount: payload.amount }); if (to === "PACKET_SENT" && payload.bettingClosed === true) addRoomMessage("ROUND", `✅ 下注意结束，平台红包已向本局 ${Number(payload.bettorCount ?? 0)} 位已下注玩家私发。旁观者不会收到领取入口。`, { amount: payload.amount, packetId: payload.packetId, bettorCount: payload.bettorCount, packetMode: "INTERNAL" }); if (to === "EVALUATING" && typeof payload.claimedAt === "string") addRoomMessage("PACKET", `${messageActor(actor)} 已领取平台红包，进入算牌。`, { claimSequence: payload.claimSequence }, actor); if (to === "ROUND_COMPLETE") addRoomMessage("SETTLEMENT", `平台通知：回合已完成，结算结果已写入 Demo 账本。`, { outcome: payload.outcome }); broadcastRoundEvent(event); return event; }
...
async function verificationState(userId: string): Promise<VerificationSnapshot> {
  ...
  const initial: VerificationSnapshot = { status: "UNVERIFIED" };
...
function addRoomMessage(type: string, message: string, payload: Record<string, unknown> = {}, userId?: string) {
  const entry: RoomMessage = { id: `MSG-${String(roomMessages.length + 1).padStart(4, "0")}`, type, body: message, createdAt: now(), payload, ...(userId ? { actor: messageActor(userId) } : {}) };
  roomMessages.push(entry);
  broadcastRoomEvent(entry);
  persistAsync(() => persistence.persistRoomMessage({ type, body: message, payload, userId }));
  queueOutbox("TELEGRAM_GROUP_ROOM_MESSAGE", { messageId: entry.id, roundId: state.round.id, type, body: message, payload });
}
async function executeBid(identity: { userId: string }, key: string, amount: number) {
  ...
  const banker = amount > currentHighest ? identity.userId : "demo-banker-01";
  state.round.banker = banker;
  if (banker === identity.userId) state.user.role = "BANKER";
  await transition("BETTING", identity.userId, { amount, currentHighest, banker, idempotencyKey: key, bankerConfirmedAt: now() });
  return { state: state.round.state, banker, amount, currentHighest };
}
async function executeBet(identity: { userId: string }, key: string, amount: number) {
  ...
  if (state.user.role === "BANKER") throw new Error("Bankers cannot place player bets");
  ...
  const bettors = roundBettors.get(state.round.id) ?? new Set<string>();
  bettors.add(identity.userId);
...
async function executeCloseBetting(identity: { userId: string }, key: string) {
  ...
  const bettorRows = persistedBettors.length > 0 ? persistedBettors : [...(roundBettors.get(state.round.id) ?? new Set<string>())].map((userId) => ({ userId, amount: 0 }));
  ...
  await transition("PACKET_SENT", identity.userId, { amount: packet.totalAmount, packetId: packet.id, maxClaims: packet.maxClaims, bettorCount: bettorRows.length, bettingClosed: true, acceptedAt: now(), packetMode: "INTERNAL" });
  await transition("CLAIMING", identity.userId, { packetId: packet.id, maxClaims: packet.maxClaims });
  for (const bettor of bettorRows) queueOutbox("ROUND_PACKET_AVAILABLE", { notificationId: `packet:${state.round.id}:${packet.id}:${bettor.userId}`, roundId: state.round.id, packetId: packet.id, amount: packet.totalAmount, maxClaims: packet.maxClaims, recipients: [bettor.userId] });
  state.round.endsAt = "00:45";
  audit(identity.userId, "BETTING_CLOSED", "ROUND", state.round.id, undefined, { packetId: packet.id, bettorCount: bettorRows.length, amount: packet.totalAmount });
...
export const apiHandler = async (request: IncomingMessage, response: ServerResponse) => {
  try {
    ...
    if (request.method === "GET" && (healthPath === "/health/ready" || healthPath === "/health")) { const databaseHealth = await persistence.health(); const worker = await workerHealth(); const botConfigured = Boolean(process.env.TELEGRAM_BOT_TOKEN); const groupChatConfigured = Boolean(process.env.TELEGRAM_GAME_GROUP_CHAT_ID); const workerRequired = process.env.REQUIRE_WORKER === "true"; const ready = appMode === "demo" ? true : databaseHealth === "healthy" && botConfigured && groupChatConfigured && (!workerRequired || worker.status === "healthy"); return json(response, ready ? 200 : 503, { ok: ready, mode: appMode, realMoneyDisabled, services: { api: ready ? "healthy" : "degraded", worker: worker.status, workerRequired, bot: botConfigured ? "configured" : "blocked", groupChat: groupChatConfigured ? "configured" : "blocked", ledger: "balanced", packetProvider: packetProvider.status, database: databaseHealth }, auth: { telegramSignedDataRequiredOutsideDemo: true, mockEnabled: telegramMockEnabled } }); }
    ...
    if (request.method === "POST" && url.pathname === "/api/verification/submit") { const identity = requirePlayer(request, response); if (!identity) return undefined; return writeIdempotent(request, response, async (key) => { const data = verificationInput(await body(request)); const current = await verificationState(identity.userId); if (current.status === "APPROVED") throw new Error("实名认证已经通过"); const next: VerificationSnapshot = persistence.configured ? await persistence.persistVerification(identity.userId, data.legalName, data.tngAccountNo) : { status: "PENDING", submittedAt: now(), tngAccountLast4: maskTngAccount(data.tngAccountNo) }; verificationStates.set(identity.userId, next); audit(identity.userId, "IDENTITY_VERIFICATION_SUBMITTED", "IDENTITY_VERIFICATION", identity.userId, undefined, { status: next.status, tngAccountLast4: next.tngAccountLast4, idempotencyKey: key }); addRoomMessage("SYSTEM", "平台通知：新的玩家已提交实名认证，审核通过后可进入游戏聊天室。", { status: "PENDING" }); return next; }); }
    if (request.method === "POST" && url.pathname.startsWith("/api/admin/verification/") && url.pathname.endsWith("/review")) { if (!requireAdmin(request)) return json(response, 403, { error: "Admin approval required" }); const telegramUserId = decodeURIComponent(url.pathname.split("/")[4] ?? ""); const data = await body(request); const status = data.status === "REJECTED" ? "REJECTED" : "APPROVED"; const reason = typeof data.reason === "string" ? data.reason.slice(0, 200) : undefined; const previous = await verificationState(telegramUserId); const next: VerificationSnapshot = persistence.configured ? (await persistence.reviewVerification(telegramUserId, status, "demo-admin", reason) ?? { status: "UNVERIFIED" }) : { status, ...(reason ? { rejectionReason: reason } : {}) }; verificationStates.set(telegramUserId, next); if (previous.status !== "APPROVED" && next.status === "APPROVED") queueOutbox("IDENTITY_VERIFICATION_APPROVED", { telegramUserId, locale: preferredLocales.get(telegramUserId) ?? "zh-CN", notificationId: `verification:${telegramUserId}:${next.submittedAt ?? now()}` }); return json(response, 200, next); }
    ...
    if (request.method === "GET" && url.pathname === "/api/rooms/room-12") return json(response, 200, { id: "room-12", name: "Project 12 Social Table", players: state.round.players, state: state.round.state, roundId: state.round.id, minDemoCredit: 250 });
     if (request.method === "GET" && url.pathname === "/api/chat/room") { const identity = await requireVerifiedPlayer(request, response); if (!identity) return undefined; const messages = persistence.configured ? await persistence.loadRoomMessages() : roomMessages; return json(response, 200, { roomId: "room-12", roomName: "十二牛牛游戏群", roundId: state.round.id, state: state.round.state, banker: state.round.banker, bankPool: state.round.bankPool, messages }); }
     if (request.method === "GET" && url.pathname === "/api/chat/room/realtime") { const identity = await requireVerifiedPlayer(request, response); if (!identity) return undefined; response.writeHead(200, { "content-type": "text/event-stream; charset=utf-8", "cache-control": "no-cache", connection: "keep-alive", "access-control-allow-origin": process.env.CORS_ORIGIN ?? "http://localhost:4173", "access-control-allow-credentials": "true" }); const messages = persistence.configured ? await persistence.loadRoomMessages() : roomMessages; for (const message of messages) writeRoomEvent(response, message); response.write(`event: snapshot\ndata: ${JSON.stringify({ roomId: "room-12", roundId: state.round.id, state: state.round.state, banker: state.round.banker, bankPool: state.round.bankPool })}\n\n`); if (url.searchParams.get("snapshot") === "1") { response.end(); return; } roomRealtimeClients.add(response); const heartbeat = setInterval(() => { if (!response.writableEnded && !response.destroyed) response.write(": heartbeat\n\n"); }, 15_000); response.on("close", () => { clearInterval(heartbeat); roomRealtimeClients.delete(response); }); return; }
     if (request.method === "POST" && url.pathname === "/api/chat/room/command") { const identity = await requireVerifiedPlayer(request, response); if (!identity) return undefined; return writeIdempotent(request, response, async (key) => { const data = await body(request); const text = typeof data.text === "string" ? data.text.trim().slice(0, 48) : ""; if (!text) throw new Error("请输入下注金额或游戏指令"); const numeric = /^(?:sh\s*)?(\d+)$/.exec(text.toLowerCase()); const closeCommand = /^(?:stop|close|停止下注|结束下注|发红包)$/i.test(text); if (numeric && state.round.state === "BANKER_BIDDING") { const amount = Number(numeric[1]); addRoomMessage("USER", `${messageActor(identity.userId)} ${amount}`, { command: "BID", amount }, identity.userId); return { command: "BID", result: await executeBid(identity, key, amount) }; } if (numeric && state.round.state === "BETTING") { const amount = Number(numeric[1]); addRoomMessage("USER", `${messageActor(identity.userId)} ${amount}`, { command: "BET", amount }, identity.userId); return { command: "BET", result: await executeBet(identity, key, amount) }; } if (closeCommand && state.round.state === "BETTING") { addRoomMessage("USER", `${messageActor(identity.userId)} 停止下注`, { command: "CLOSE_BETTING" }, identity.userId); return { command: "CLOSE_BETTING", result: await executeCloseBetting(identity, key) }; } if (/^(?:help|帮助|玩法)$/i.test(text)) { return { command: "HELP", result: { state: state.round.state, message: state.round.state === "BANKER_BIDDING" ? "发送数字抢庄，例如 400" : state.round.state === "BETTING" ? "发送数字下注，例如 10；庄家发送 停止下注" : "请等待本局继续" } }; } throw new Error(state.round.state === "BANKER_BIDDING" ? "抢庄阶段请输入整数庄金，例如 400" : state.round.state === "BETTING" ? "下注阶段请输入整数金额，例如 10" : "当前阶段不接受聊天室指令"); }); }
    ...
    if (request.method === "POST" && url.pathname === "/api/admin/outbox/publish") return requireAdmin(request) ? writeIdempotent(request, response, (key) => { const published = outboxEvents.filter((event) => !event.publishedAt).map((event) => { event.publishedAt = now(); return event.id; }); audit("admin", "OUTBOX_PUBLISHED", "OUTBOX", key, undefined, { count: published.length }); return { published }; }) : json(response, 403, { error: "Admin authorization required" });
     ...
     if (request.method === "POST" && url.pathname === `/api/rounds/${state.round.id}/bid`) { const identity = requirePlayer(request, response); if (!identity) return undefined; return writeIdempotent(request, response, async (key) => executeBid(identity, key, Number((await body(request)).amount ?? 100))); }
     if (request.method === "POST" && url.pathname === `/api/rounds/${state.round.id}/bet`) { const identity = requirePlayer(request, response); if (!identity) return undefined; return writeIdempotent(request, response, async (key) => executeBet(identity, key, Number((await body(request)).amount ?? 250))); }
     if (request.method === "POST" && url.pathname === `/api/rounds/${state.round.id}/close-betting`) { const identity = requirePlayer(request, response); if (!identity) return undefined; return writeIdempotent(request, response, async (key) => executeCloseBetting(identity, key)); }
    if (request.method === "GET" && url.pathname === `/api/rounds/${state.round.id}/packet`) { const identity = await requireVerifiedPlayer(request, response); if (!identity) return undefined; const packet = await packetProvider.getPacket(state.round.id); if (packet) packetIds.set(state.round.id, packet.id); const persistedBettors = persistence.configured ? await persistence.listRoundBettors() : []; const bettorIds = persistedBettors.length > 0 ? persistedBettors.map((row) => row.userId) : [...(roundBettors.get(state.round.id) ?? new Set<string>())]; return json(response, 200, { state: state.round.state, packet, eligible: bettorIds.includes(identity.userId), bettorCount: bettorIds.length }); }
    if (request.method === "POST" && [`/api/rounds/${state.round.id}/packet-claim`, `/api/rounds/${state.round.id}/demo-claim`].includes(url.pathname)) { const identity = requirePlayer(request, response); if (!identity) return undefined; return writeIdempotent(request, response, async (key) => { if (!realMoneyDisabled) throw new Error("Unexpected money mode"); if (packetProvider.status !== "DEMO_READY") throw new Error(`Packet provider unavailable: ${packetProvider.name}`); if (!["CLAIMING", "EVALUATING"].includes(state.round.state)) throw new Error("Claim window is closed"); const persistedBettors = persistence.configured ? await persistence.listRoundBettors() : []; const bettorIds = persistedBettors.length > 0 ? persistedBettors.map((row) => row.userId) : [...(roundBettors.get(state.round.id) ?? new Set<string>())]; if (!bettorIds.includes(identity.userId)) throw new PacketProviderError("AUTHORIZATION_REQUIRED", "只有本局已下注玩家可以领取平台红包"); const packetId = packetIds.get(state.round.id) ?? (await packetProvider.getPacket(state.round.id))?.id; if (!packetId) throw new Error("平台红包尚未发放"); packetIds.set(state.round.id, packetId); const claim = await packetProvider.claim({ packetId, serverSeed, roundId: state.round.id, userId: identity.userId, claimSequence: 1 }); await persistence.persistPacketClaim(identity.userId, claim.claimSequence, claim.value); if (state.round.state === "CLAIMING") await transition("EVALUATING", identity.userId, { claimSequence: claim.claimSequence, claimedAt: claim.claimedAt, idempotencyKey: key }); state.round.endsAt = "Done"; const hand = classifyHand([3, 4, 2]); audit(identity.userId, "INTERNAL_PACKET_CLAIMED", "ROUND", state.round.id, undefined, claim); return { ...claim, hand }; }); }
