...
function roomLeaderboard(category: "points" | "cards" | "banker") {
  ...
  const rows = (state.leaderboard ?? []).map((entry, index) => {
    ...
    return { rank: index + 1, userId: entry.userId, displayName: entry.displayName, value, unit: category === "points" ? "PT" : category === "cards" ? "SCORE" : "BIDS", source: category === "banker" && bidCounts.has(entry.userId) ? "CURRENT_ROUND" : "DEMO_SEEDED" };
  }).sort((left, right) => right.value - left.value || left.rank - right.rank).map((entry, index) => ({ ...entry, rank: index + 1 }));
  return { roomId: "room-12", category, entries: rows, empty: rows.length === 0, demoOnly: appMode === "demo" };
}
function dailyRewardSnapshot() {
  ...
  return { roomId: "room-12", date: new Date().toISOString().slice(0, 10), claims: [...dailyRewardClaims], rewards, demoOnly: appMode === "demo" };
}
...
async function addRoomMessage(type: string, message: string, payload: Record<string, unknown> = {}, userId?: string, visibility: "PUBLIC_ROOM" | "PARTICIPANTS_ONLY" | "TARGET_USER" | "ADMIN_ONLY" = "PUBLIC_ROOM", targetUserId?: string): Promise<void> {
  ...
  const createdAt = now();
  const persisted = await persistence.persistRoomMessage({ type, body: message, payload, templateKey, userId, visibility, targetUserId });
...
export const apiHandler = async (request: IncomingMessage, response: ServerResponse) => {
  try {
    ...
    if (request.method === "GET" && url.pathname === "/api/realtime/token") {
      ...
      if (!identity) return undefined;
      await persistence.ensureRoomMember(identity.userId);
    ...
    if (request.method === "GET" && url.pathname === "/api/admin/rules") return requireAdmin(request) ? json(response, 200, [{ id: demoRules.id, version: demoRules.id, status: "ACTIVE", rules: demoRules, immutable: true, demoOnly: true }]) : json(response, 403, { error: "Admin authorization required" });
