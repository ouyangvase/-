...
export const apiHandler = async (request: IncomingMessage, response: ServerResponse) => {
  try {
    ...
    if (request.method === "POST" && url.pathname === `/api/rounds/${state.round.id}/bet`) { const identity = requirePlayer(request, response); if (!identity) return undefined; return writeIdempotent(request, response, async (key) => { const data = await body(request); const amount = Number(data.amount ?? 250); if (!Number.isInteger(amount) || amount <= 0) throw new Error("Bet amount must be a positive integer"); if (state.round.state !== "BETTING") throw new Error("Betting is closed"); if (state.user.role === "BANKER") throw new Error("Bankers cannot place player bets"); const journal = createTransferJournal({ id: `J-BET-${identity.userId}`, referenceType: "BET_LOCK", referenceId: state.round.id, idempotencyKey: key, reason: "Lock demo bet", from: "USER_AVAILABLE", to: "USER_LOCKED", amount }); Object.assign(balances, applyJournal(balances, journal)); await persistence.persistBet(identity.userId, amount); await persistence.persistJournal(journal, identity.userId); const bettors = roundBettors.get(state.round.id) ?? new Set<string>(); bettors.add(identity.userId); roundBettors.set(state.round.id, bettors); syncUserBalances(); addRoomMessage("BET", `${messageActor(identity.userId)} 下单 ${amount} PT，等待停止下注。`, { amount, packetMode: "PENDING" }, identity.userId); state.ledger.unshift({ id: journal.id, reason: journal.reason, change: -amount, balanceAfter: state.user.available, createdAt: "Just now" }); audit(identity.userId, "BET_LOCKED", "JOURNAL", journal.id, undefined, { journal, bettingClosed: false }); return { state: state.round.state, locked: state.user.locked, journal, bettingClosed: false, packet: null }; }); }
    if (request.method === "POST" && url.pathname === `/api/rounds/${state.round.id}/close-betting`) { const identity = requirePlayer(request, response); if (!identity) return undefined; return writeIdempotent(request, response, async (key) => { if (state.round.state !== "BETTING") throw new Error("下注阶段已经结束"); const persistedBettors = persistence.configured ? await persistence.listRoundBettors() : []; const bettorRows = persistedBettors.length > 0 ? persistedBettors : [...(roundBettors.get(state.round.id) ?? new Set<string>())].map((userId) => ({ userId, amount: 0 })); if (bettorRows.length === 0) throw new Error("至少需要一位下注玩家才能发放红包"); const totalBets = bettorRows.reduce((sum, row) => sum + row.amount, 0); const totalAmount = Math.max(bettorRows.length, totalBets || balances.USER_LOCKED || 1); const packet = await packetProvider.createPacket({ roundId: state.round.id, amount: totalAmount, maxClaims: bettorRows.length, serverSeedHash: hashSeed(serverSeed) }); packetIds.set(state.round.id, packet.id); await persistence.persistPacket(packet.provider, hashSeed(serverSeed)); await transition("PACKET_SENT", identity.userId, { amount: packet.totalAmount, packetId: packet.id, maxClaims: packet.maxClaims, bettorCount: bettorRows.length, bettingClosed: true, acceptedAt: now(), packetMode: "INTERNAL" }); await transition("CLAIMING", identity.userId, { packetId: packet.id, maxClaims: packet.maxClaims }); for (const bettor of bettorRows) queueOutbox("ROUND_PACKET_AVAILABLE", { notificationId: `packet:${state.round.id}:${packet.id}:${bettor.userId}`, roundId: state.round.id, packetId: packet.id, amount: packet.totalAmount, maxClaims: packet.maxClaims, recipients: [bettor.userId] }); state.round.endsAt = "00:45"; audit(identity.userId, "BETTING_CLOSED", "ROUND", state.round.id, undefined, { packetId: packet.id, bettorCount: bettorRows.length, amount: packet.totalAmount }); return { state: state.round.state, packet, bettorCount: bettorRows.length, bettingClosed: true }; }); }
