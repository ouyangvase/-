...
async function hydrateRuntime(runtime: ApiRuntime): Promise<void> {
  ...
  if (snapshot) {
    ...
    runtime.state.round.players = snapshot.playerCount;
    runtime.state.round.ruleVersion = snapshot.ruleVersion;
    runtime.state.round.seedHash = snapshot.serverSeedHash ? `${snapshot.serverSeedHash.slice(0, 16)}…` : "—";
...
export const apiHandler = async (request: IncomingMessage, response: ServerResponse) => {
  try {
    ...
    const healthPath = url.pathname.replace(/^\/api(?=\/|$)/, "").replace("/health/app", "/health/ready");
    ...
    if (request.method === "GET" && healthPath === "/health/realtime") { const configured = hasSupabaseRealtime(); return json(response, configured ? 200 : 503, { ok: configured, service: "realtime", configured }); }
    if (request.method === "GET" && healthPath === "/health/session") return activeSession ? json(response, 200, { ok: true, service: "session", userId: activeSession.userId }) : json(response, 401, { ok: false, code: "SESSION_NOT_CREATED", error: "App session is not established" });
