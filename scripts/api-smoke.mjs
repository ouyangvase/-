...
if (health.status !== 200 || auth.status !== 200 || bid.status !== 200 || bid.data.result?.state !== "BETTING" || bet.status !== 200 || replay.status !== 200 || replay.data.replayed !== true || claim.status !== 200 || settle.status !== 200 || settle.data.result?.settlement?.playerCredit !== 2387.5 || settle.data.result?.journal?.lines?.some((line) => line.amount === 112.5) !== true || money.status !== 403 || money.data.code !== "REAL_MONEY_DISABLED" || device.status !== 200 || referrer.status !== 200 || pin.status !== 200 || onboarding.data.pinSet !== true) {
  console.error("API smoke failed", { health, auth, bid, bet, replay, claim, settle, money, device, referrer, pin, onboarding });
  process.exit(1);
}
console.log(JSON.stringify({ health, bidStatus: bid.status, betStatus: bet.status, replayed: replay.data.replayed, claimStatus: claim.status, settleStatus: settle.status, playerCredit: settle.data.result?.settlement?.playerCredit, feeLine: settle.data.result?.journal?.lines?.find((line) => line.account === "PLATFORM_FEE")?.amount, moneyStatus: money.status, moneyCode: money.data.code }, null, 2));
